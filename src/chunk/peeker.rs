...
pub struct Peeker<B: ?Sized> {
  ...
  start: Bound<usize>,
  /// The end bound that [`reset`](Peeker::reset) restores to. Initialized to the
...
impl<B: Chunk> Chunk for Peeker<B> {
  ...
  ///
  /// This also re-anchors the reset target (`end`), so a later [`reset`](Peeker::reset)
...
mod tests {
  ...
  fn test_peeker_panic_safety_out_of_range_bounds() {
    ...
    let mut p = Peeker::with_limit(buf, len + 3);
    ...
    // slicing `&buf[5..2]`).
    let mut p = Peeker::with_range(buf, 5..2);
    assert_eq!(p.buffer().len(), p.remaining());
    assert_eq!(p.remaining(), 0);
    ...
    let mut p = Peeker::with_range(buf, ..=usize::MAX);
    assert_eq!(p.buffer().len(), p.remaining());
    assert_eq!(p.remaining(), len);
    assert_eq!(p.peek_u8_checked(), Some(1));
    ...
    let mut p = Peeker::with_range(buf, 0..(len + 5));
    assert_eq!(p.buffer().len(), p.remaining());
    assert_eq!(p.remaining(), len);
    assert_eq!(p.peek_u8_checked(), Some(1));
    assert!(p.try_advance(len).is_ok());
